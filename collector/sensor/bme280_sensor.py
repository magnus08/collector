...
def _populate_calibration_data():
    raw_data = []
...
def _read_pressure(data=None):
    ...
    _read_temperature(data)
    return _compensate_pressure(data.pressure)


def _read_temperature(data=None):
    if data is None:
        data = _read_adc()

    return _compensate_temperature(data.temperature)


def _compensate_pressure(adc_p):
...
def _compensate_temperature(adc_t):
    global t_fine
...
def _compensate_humidity(adc_h):
